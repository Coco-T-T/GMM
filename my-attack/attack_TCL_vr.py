...
def my_text_attack(text_attacker, model, ref_tokenizer, texts, images, criterion, device, sample_num=10, rate=0.7):
    # model_ALBEF  target
    # ref_model  generator
    # tokenizer  
    ...
    adv_text_bank = text_attacker.attack(model, texts, sample_num=sample_num)   # generate N adv tests
    ...
    for i in range(len(texts)):
        ...
        for j in range(len(adv_text_bank[i])):
            ...
            loss1 = criterion(adv_embed.log_softmax(dim=-1), image_embeds[i].softmax(dim=-1))   # distance between adv text and adv image: up
            loss2 = criterion(adv_embed.log_softmax(dim=-1), text_embeds[i].softmax(dim=-1))   # distance between adv text and ori text: down
            score.append(loss1 * rate - loss2 * (1-rate))   # Equation 4
...
def my_image_text_attack(model, ref_model, data_loader, ref_tokenizer, device, config, mod):
    ...
    for images, texts, texts_ids in metric_logger.log_every(data_loader, 10, 'Evaluation:'):
        ...
        if mod:
            ...
            origin_embeds = origin_output['image_embed'][:, 0, :].detach()
            text_input = ref_tokenizer(texts, padding='max_length', truncation=True, max_length=30, return_tensors="pt").to(device)
            text_output = model.inference_text(text_input)
            ...
            for i in range(num_iters):
                ...
                adv_embeds = adv_output['image_embed'][:, 0, :]

                loss1 = criterion(adv_embeds.log_softmax(dim=-1), origin_embeds.softmax(dim=-1))   # distance between adv image and ori image
                loss2 = criterion(F.normalize(adv_embeds, dim=-1).log_softmax(dim=-1), 
                                  F.normalize(text_embed, dim=-1).softmax(dim=-1))   # distance between adv image and ori text
                loss = loss1 + alpha * loss2   # Equation 2
                loss.backward()
...
def main(args, config):
    ...
    checkpoint = torch.load('TCL-mscoco.pth', map_location='cpu')
    try:
    ...
    with open(os.path.join(args.output_dir, "log_TCL.txt"), "a+") as f:
