...
def my_text_attack(text_attacker, model_ALBEF, ref_tokenizer, texts, images, criterion, device, sample_num=10, rate=0.7):
    ...
    adv_text_bank = text_attacker.attack(model_ALBEF, texts, sample_num=sample_num)   # generate N adv tests
        ...
        if mod:
            ...
            for i in range(num_iters):
                ...
                loss2 = criterion(F.normalize(adv_embeds_, dim=-1).log_softmax(dim=-1), 
                                  F.normalize(text_embeds, dim=-1).softmax(dim=-1))   # distance between adv image and ori text
...
def main(args, config):
    ...
    checkpoint = torch.load("TCL-VE.pth", map_location='cpu')
    try:
